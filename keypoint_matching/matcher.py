...
class Matcher(object):
    ...
    def vis_matches(self, img1, img2, flag=0, show_start=0, show_end=50):
        ...
        if self.key_pt_pair.bbox1.top_left_x == 0 and self.key_pt_pair.bbox1.top_left_y == 0 \
            ...
            out = cv2.drawMatches(img1, self.key_pt_pair.kp1, img2, self.key_pt_pair.kp2,
                                  self.matches[show_start:show_end], out, flags=2)
    ...
    def write_matches(self, img1, img2, match_path, flag=0, show_start=0, show_end=50):
        ...
        if self.key_pt_pair.bbox1.top_left_x == 0 and self.key_pt_pair.bbox1.top_left_y == 0 \
            ...
            height, width, _ = img1.shape
            out = np.zeros((width * 2, height))
            out = cv2.drawMatches(img1, self.key_pt_pair.kp1, img2, self.key_pt_pair.kp2, self.matches, out, flags=2)
